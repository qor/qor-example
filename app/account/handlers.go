...
func (ctrl Controller) Orders(w http.ResponseWriter, req *http.Request) {
	...
	tx.Preload("OrderItems").Preload("OrderItems.SizeVariation.Size").Preload("OrderItems.SizeVariation.ColorVariation.Color").Preload("OrderItems.SizeVariation.ColorVariation.Product").Where(&orders.Order{UserID: currentUser.ID}).Find(&Orders)
